...
// -------------------------
// Ingredient substitutions (lazy OpenAI import)
// -------------------------
app.get("/api/ingredients/:ingredient/substitutions", async (req: Request, res: Response) => {
  try {
    const ingredient = String(req.params.ingredient || "").trim();
    if (!ingredient) return res.status(400).json({ error: "Ingredient is required" });

    // If no key, return static suggestions (no dependency on openai package)
